...
{
    public class IceBarrier : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Projectile.timeLeft == 24)
            {
                for (int i = 0; i < 50; i++)
                {
                    ...
                    Dust d = Dust.NewDustPerfect(Projectile.Center, ModContent.DustType<SnowRing>(), speed * -18, Scale: 5.1f);
                    d.noGravity = true;
                }
                for (int i = 0; i < 50; i++)
                {
                    Vector2 speed = Main.rand.NextVector2CircularEdge(.35f, .35f);
                    Dust d = Dust.NewDustPerfect(Projectile.Center, ModContent.DustType<SnowRingFog>(), speed * 20, Scale: 5.1f);
                    d.noGravity = true;
                }
        ...
        public override void ModifyHitNPC(NPC target, ref int damage, ref float knockback, ref bool crit, ref int hitDirection)
        {
            ...
            modPlayer.SariaXp++;
            knockback = 1f;
            if (!target.boss && !target.HasBuff(ModContent.BuffType<EnemyFrozen>()))
            {
