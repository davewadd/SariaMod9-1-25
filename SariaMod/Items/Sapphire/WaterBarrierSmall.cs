...
namespace SariaMod.Items.Sapphire
{
    public class WaterBarrierSmall : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            int Yesh = ((player2.statManaMax2) / 10);
            int Yesh2 = ((player2.statManaMax2) / 7);
            ...
            Projectile.damage /= 6;
            {
                if (player2.Hitbox.Intersects(Projectile.Hitbox) && player2.active && (player2.team == player.team) && !player2.HasBuff(ModContent.BuffType<Healed2>()))
                {
                    if ((player2.statLife < player2.statLifeMax2) || (player2.statMana < player2.statManaMax2))
                    {
                        player2.AddBuff(ModContent.BuffType<Healed2>(), 30);
                        for (int i = 0; i < 50; i++)
                        {
                            Vector2 speed = Main.rand.NextVector2CircularEdge(1f, 1f);
