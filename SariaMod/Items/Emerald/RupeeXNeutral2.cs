...
{
    public class RupeeXNeutral2 : ModProjectile
    {
        ...
        {
            Projectile.Rupeedraw(((Texture2D)ModContent.Request<Texture2D>("SariaMod/Items/Emerald/RupeeXNeutral2").Value), lightColor, Color.GhostWhite, 1);
        }
        public override void Kill(int timeLeft)
        {
            ...
            {
                if (Main.myPlayer == Projectile.owner) Projectile.NewProjectile(Projectile.GetSource_FromThis(), player.Center.X - 50, player.Center.Y + 150, 0, 0, ModContent.ProjectileType<RupeeXPassive2>(), (int)(Projectile.damage), 0f, Projectile.owner, player.whoAmI, base.Projectile.whoAmI);
                player.AddBuff(ModContent.BuffType<EmeraldBuff>(), 40);
                Projectile.timeLeft = 2;
            }
            else if (player.ownedProjectileCounts[ModContent.ProjectileType<RupeeXPassive2>()] > 0f)
            {
                SoundEngine.PlaySound(SoundID.DD2_WitherBeastCrystalImpact, Projectile.Center);
                Lighting.AddLight(Projectile.Center, Color.Purple.ToVector3() * 6f);
                for (int i = 0; i < 2; i++)
                {
                    if (Main.myPlayer == Projectile.owner) Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.position.X + 0, Projectile.position.Y - 0, 0, 0, ModContent.ProjectileType<RupeeShard2>(), (int)(Projectile.damage), 0f, Projectile.owner, player.whoAmI, base.Projectile.whoAmI);
                }
                for (int i = 0; i < 50; i++)
                {
                    Vector2 speed = Main.rand.NextVector2CircularEdge(1f, 1f);
