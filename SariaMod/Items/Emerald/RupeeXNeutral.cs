...
{
    public class RupeeXNeutral : ModProjectile
    {
        ...
        {
            Projectile.Rupeedraw(((Texture2D)ModContent.Request<Texture2D>("SariaMod/Items/Emerald/RupeeXNeutral").Value), lightColor, Color.GhostWhite, 1);
        }
        public override void Kill(int timeLeft)
        {
            ...
            {
                if (Main.myPlayer == Projectile.owner) Projectile.NewProjectile(Projectile.GetSource_FromThis(), player.Center.X - 50, player.Center.Y + 150, 0, 0, ModContent.ProjectileType<RupeeXPassive>(), (int)(Projectile.damage), 0f, Projectile.owner, player.whoAmI, base.Projectile.whoAmI);
                player.AddBuff(ModContent.BuffType<EmeraldBuff>(), 40);
                Projectile.timeLeft = 2;
            }
            else if (player.ownedProjectileCounts[ModContent.ProjectileType<RupeeXPassive>()] > 0f)
            {
                SoundEngine.PlaySound(SoundID.DD2_WitherBeastCrystalImpact, Projectile.Center);
                Lighting.AddLight(Projectile.Center, Color.Green.ToVector3() * 6f);
                for (int i = 0; i < 1; i++)
                {
                    if (Main.myPlayer == Projectile.owner) Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.position.X + 0, Projectile.position.Y - 0, 0, 0, ModContent.ProjectileType<RupeeShard>(), (int)(Projectile.damage), 0f, Projectile.owner, player.whoAmI, base.Projectile.whoAmI);
                }
                for (int i = 0; i < 50; i++)
                {
                    Vector2 speed = Main.rand.NextVector2CircularEdge(1f, 1f);
