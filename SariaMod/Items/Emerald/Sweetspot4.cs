...
{
    public class Sweetspot4 : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (player.velocity.Y <= 0 || player.ownedProjectileCounts[ModContent.ProjectileType<Emeraldspike3_2 >()] < 1f)
            {
                if (Main.myPlayer == Projectile.owner && !CanSee2) Projectile.NewProjectile(Projectile.GetSource_FromThis(), player.Center.X, player.Center.Y + 20, 0, 0, ModContent.ProjectileType<LaunchHitBox3>(), (int)(Projectile.damage), 0f, Projectile.owner, player.whoAmI, base.Projectile.whoAmI);
                Projectile.Kill();
            }
