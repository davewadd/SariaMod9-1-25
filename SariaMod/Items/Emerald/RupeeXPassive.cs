...
{
    public class RupeeXPassive : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (player.ownedProjectileCounts[ModContent.ProjectileType<Change1>()] > 0f)
            {
            ...
            if (player.ownedProjectileCounts[ModContent.ProjectileType<Change1>()] <= 0f && ChangeSoundtimer)
            {
                ChangeSoundtimer = false;
            }
            if (player.ownedProjectileCounts[ModContent.ProjectileType<Change2>()] > 0f && !ChangeSoundtimer2)
            {
                IsActive = false;
            ...
            if (Cooldown >= 1 && (player.ownedProjectileCounts[ModContent.ProjectileType<Emeraldspike>()] <= 0f))
            {
                Cooldown--;
            }
            ...
            if (IsActive && player.controlDown && player.releaseJump && Cooldown <= 0 && (player.ownedProjectileCounts[ModContent.ProjectileType<Emeraldspike>()] <= 0f))
            {
                if (Main.myPlayer == Projectile.owner) Projectile.NewProjectile(Projectile.GetSource_FromThis(), player.Center.X + 0, player.Center.Y - 60, 0, 0, ModContent.ProjectileType<Emeraldspike>(), (int)(Projectile.damage), 0f, Projectile.owner, player.whoAmI, base.Projectile.whoAmI);
        ...
        public override void PostDraw(Color lightColor)
        {
            ...
            if (between <= yup && Main.myPlayer == Projectile.owner)
            {
                ...
                if (HasClickedTimer < 20)
                {
                    player.mouseInterface = true;
                    {
                        ...
                        {
                            HasClicked = true;
                            if (Main.myPlayer == Projectile.owner) Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center.X + 0, Projectile.Center.Y - 0, 0, 0, ModContent.ProjectileType<Change2>(), (int)(Projectile.damage), 0f, Projectile.owner, player.whoAmI, base.Projectile.whoAmI);
                        }
                    }
                    if (Soundtimer)
                    {
            ...
            if (IsActive)
            {
                Texture2D texture = ((Texture2D)ModContent.Request<Texture2D>("SariaMod/Items/Emerald/RupeeXPassive").Value);
            ...
            Projectile.Rupeedraw(((Texture2D)ModContent.Request<Texture2D>("SariaMod/Items/Emerald/RupeeXPassive").Value), lightColor, Color.GhostWhite, 3);
            Projectile.RupeeGlowandFadedraw(((Texture2D)ModContent.Request<Texture2D>("SariaMod/Items/Emerald/RupeeMask").Value), lightColor, Color.GhostWhite, 3);
        }
        public override void Kill(int timeLeft)
        {
            ...
            {
                int dropItemType = ModContent.ItemType<Items.Emerald.LivingGreenShard>(); // This the item we want the paper airplane to drop.
            ...
            {
                int dropItemType = ModContent.ItemType<Items.Emerald.LivingGreenFragment>(); // This the item we want the paper airplane to drop.
                int newItem = Item.NewItem(Projectile.GetSource_DropAsItem(), Projectile.Hitbox, dropItemType); // Create a new item in the world.
