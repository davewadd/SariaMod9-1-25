...
{
    public class RupeeXPassive2 : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (player.ownedProjectileCounts[ModContent.ProjectileType<Change1_2>()] > 0f)
            {
                for (int U = 0; U < 1000; U++)
                {
                    if (Main.projectile[U].active && Main.projectile[U].ModProjectile is RupeeXPassive modRupee && U != Projectile.whoAmI && ((Main.projectile[U].owner == owner)))
                    {
                        ...
                        if (!ChangeSoundtimer)
                        {
                            SoundEngine.PlaySound(new SoundStyle("SariaMod/Sounds/SilverRupee2"), Projectile.Center);
                            for (int i = 0; i < 50; i++)
                            {
                    ...
                    if (Main.projectile[U].active && Main.projectile[U].ModProjectile is RupeeXPassive3 modRupee3 && U != Projectile.whoAmI && ((Main.projectile[U].owner == owner)))
                    {
                        IsActive = true;
                        modRupee3.IsActive = false;
                        if (!ChangeSoundtimer)
                        {
                            SoundEngine.PlaySound(new SoundStyle("SariaMod/Sounds/SilverRupee2"), Projectile.Center);
                            for (int i = 0; i < 50; i++)
            ...
            if (player.ownedProjectileCounts[ModContent.ProjectileType<Change1_2>()] <= 0f && ChangeSoundtimer)
            {
                ChangeSoundtimer = false;
            }
            if (player.ownedProjectileCounts[ModContent.ProjectileType<Change2_2>()] > 0f && !ChangeSoundtimer2)
            {
                IsActive = false;
            ...
            if (Cooldown >= 1 && (player.ownedProjectileCounts[ModContent.ProjectileType<Emeraldspike2>()] <= 0f))
            {
                Cooldown--;
            }
            ...
            if (IsActive && player.controlDown && player.releaseJump && Cooldown <= 0 && (player.ownedProjectileCounts[ModContent.ProjectileType<Emeraldspike2>()] <= 0f))
            {
                if (Main.myPlayer == Projectile.owner) Projectile.NewProjectile(Projectile.GetSource_FromThis(), player.Center.X + 0, player.Center.Y - 60, 0, 0, ModContent.ProjectileType<Emeraldspike2>(), (int)(Projectile.damage), 0f, Projectile.owner, player.whoAmI, base.Projectile.whoAmI);
            ...
            if (player.dead || !player.active)
            {
                ...
                {
                    Item.NewItem(Projectile.GetSource_FromThis(), Projectile.Center + Utils.RandomVector2(Main.rand, -24f, 24f), ModContent.ItemType<LivingPurpleShard>());
                }
                for (int i = 0; i < 50; i++)
                {
                    Vector2 speed = Main.rand.NextVector2CircularEdge(1f, 1f);
        ...
        public override void PostDraw(Color lightColor)
        {
            ...
            if (between <= yup && Main.myPlayer == Projectile.owner)
            {
                ...
                if (HasClickedTimer < 20)
                {
                    player.mouseInterface = true;
                    {
                        ...
                        {
                            HasClicked = true;
                            if (Main.myPlayer == Projectile.owner) Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center.X + 0, Projectile.Center.Y - 0, 0, 0, ModContent.ProjectileType<Change2_2>(), (int)(Projectile.damage), 0f, Projectile.owner, player.whoAmI, base.Projectile.whoAmI);
                        }
                    }
                    if (Soundtimer)
                    {
            ...
            if (IsActive)
            {
                Texture2D texture = ((Texture2D)ModContent.Request<Texture2D>("SariaMod/Items/Emerald/RupeeXPassive2").Value);
            ...
            Projectile.Rupeedraw(((Texture2D)ModContent.Request<Texture2D>("SariaMod/Items/Emerald/RupeeXPassive2").Value), lightColor, Color.GhostWhite, 3);
            Projectile.RupeeGlowandFadedraw(((Texture2D)ModContent.Request<Texture2D>("SariaMod/Items/Emerald/RupeeMask2").Value), lightColor, Color.GhostWhite, 3);
        }
        public override void Kill(int timeLeft)
        {
            Player player = Main.player[Projectile.owner];
            player.AddBuff(ModContent.BuffType<PurpleRupeeBlock>(), 3200);
            for (int i = 0; i < 50; i++)
            {
            ...
            {
                int dropItemType = ModContent.ItemType<Items.Emerald.LivingPurpleShard>(); // This the item we want the paper airplane to drop.
            ...
            {
                int dropItemType = ModContent.ItemType<Items.Emerald.LivingPurpleFragment>(); // This the item we want the paper airplane to drop.
                int newItem = Item.NewItem(Projectile.GetSource_DropAsItem(), Projectile.Hitbox, dropItemType); // Create a new item in the world.
            ...
            else if (player.ownedProjectileCounts[ModContent.ProjectileType<RupeeXPassive3>()] >= 1f)
            {
                for (int U = 0; U < 1000; U++)
                {
